...
def process_tmdb_data(
    ...
    get_data_func: function = table_config["getter_func"]
    ...
    for movie_id in movie_ids:  # loop through `movie_ids`
        ...
        if nested_field:  # For the tables that have nested field we loop through
            nested_values = api_record[nested_field][:limitation]
        ...
        else:  # For the tables that has no nested field we need to loop through
            row_record = {
                "movie_id": movie_id,
                "created_at_utc": datetime.now(timezone.utc),
            }
            for column, field in field_mapping.items():
                row_record[column] = api_record[field]
            response_data_raw.append(row_record)
...
def main():
    ...
    engine = get_engine()
    movie_ids = get_top_rated_movies(pages=1)
